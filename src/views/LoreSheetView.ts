...
import { t } from '../i18n';
import { LoreSheetData, CharacterImage, CustomPropertyDefinition, LORE_CATEGORIES } from '../types';
import { parseLoreSheet, serializeLoreSheet } from '../utils/loreSheetUtils';

export const LORE_SHEET_VIEW_TYPE = 'lore-sheet';
...
export class LoreSheetView extends TextFileView {
  plugin: NovalistPlugin;
  data: LoreSheetData;
  originalData: string = '';
  ...
  getViewType(): string {
    return LORE_SHEET_VIEW_TYPE;
  }

  getDisplayText(): string {
    return this.file ? this.file.basename : t('loreSheet.fallbackTitle');
  }
  ...
  getViewData(): string {
    return serializeLoreSheet(this.data);
  }

  setViewData(data: string, _clear: boolean): void {
    this.originalData = data;
    this.data = parseLoreSheet(data);
  ...
  private mergeFromTemplate(): void {
    const template = this.plugin.getLoreTemplate(this.data.templateId);
  ...
  private renderBasicInfo(container: HTMLElement): void {
    ...
    section.createEl('h3', { text: t('loreSheet.basicInfo'), cls: 'character-sheet-section-title' });

    new Setting(section)
      .setName(t('loreSheet.name'))
      .addText(text => {
    ...
    new Setting(section)
      ...
      .addDropdown(dd => {
        ...
        dd.onChange(value => { this.data.category = value; });
      });

    new Setting(section)
      .setName(t('loreSheet.description'))
      .addTextArea(text => {
        text.setValue(this.data.description || '');
        text.setPlaceholder(t('loreSheet.descriptionPlaceholder'));
  ...
  private renderCustomProperties(container: HTMLElement): void {
    const section = container.createDiv('character-sheet-section');
    section.createEl('h3', { text: t('loreSheet.customProperties'), cls: 'character-sheet-section-title' });
        ...
        case 'enum': {
          if (def?.enumOptions && def.enumOptions.length > 0) {
            ...
            const fallback = row.createEl('input', {
              type: 'text',
              cls: 'location-sheet-custom-value',
              placeholder: t('loreSheet.valuePlaceholder')
            });
    ...
    new ButtonComponent(section)
      .setButtonText(t('loreSheet.addProperty'))
      .onClick(() => {
        let i = 1;
  ...
  private renderImagesSection(container: HTMLElement): void {
    ...
    section.createEl('h3', { text: t('loreSheet.images'), cls: 'character-sheet-section-title' });

    const images = this.data.images;
    const imagesList = section.createDiv('character-sheet-images-list');

    if (images.length === 0) {
      imagesList.createEl('p', { text: t('loreSheet.dropImages'), cls: 'character-sheet-empty' });
    } else {
    ...
    new ButtonComponent(section)
      .setButtonText(t('loreSheet.addImage'))
      .onClick(() => {
  ...
  async save(): Promise<void> {
    const content = serializeLoreSheet(this.data);
    if (this.file) {
      ...
      if (nextName && this.file.basename !== nextName) {
        ...
        if (!this.app.vault.getAbstractFileByPath(nextPath)) {
          ...
          new Notice(t('notice.loreFileExists'));
        }
      }
      await this.app.vault.modify(this.file, content);
      new Notice(t('notice.loreSaved'));
    }
  }
}
