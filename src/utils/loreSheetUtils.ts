...
export function parseLoreSheet(content: string): LoreSheetData {
  ...
  const sheetContent = getSheetSection(normalized, 'LoreSheet');
  ...
  const parseField = (fieldContent: string, fieldName: string): string => {
    ...
    const knownFields = ['Name:', 'Category:', 'Description:', 'CustomProperties:', 'Sections:', 'TemplateId:', 'Images:'];
  ...
  data.category = parseField(sheetContent, 'Category');
  ...
  if (descSectionIdx !== -1) {
    ...
    const nextSections = ['Category:', 'Images:', 'CustomProperties:', 'Sections:'];
